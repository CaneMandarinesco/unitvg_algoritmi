...
def nodoToSequenza(i, n):
    # O(1)
    if(n%2 == 0):
        return 

def max_cfu(s1, s2):
    # La procedura esegue in tempo O(1)
    ...
    if (s1_k != s2_k):
        ...
        if s1_t < s2_s:
            return (s1_s, s2_t, (s1_len*s1_v) + (s2_len)*s2_v)
        ...
        if s1_t >= s2_t:
            ...
            if s1_v >= s2_v:
                return (s1_s, s1_t, s1_len*s1_v)
            # se i cfu in s1 valgono di meno
            return (s1_s, s1_t, ((s1_len-s2_len)*s1_v) + (s2_len * s2_v))
        
        # se una parte di s2 si trova in s1 e l'altra e' fuori...
        # se i cfu in s1 valgono piu di s2
        if s1_v >= s2_v:
            return (s1_s, s2_t, (s1_len * s1_v) + ((s2_t - s1_t) * s2_v))
        
        # se i cfu in s1 valgono meno di s2
        return (s1_s, s2_t, ((s2_s - s1_s) * s1_v) + (s2_len*s2_v))
...
def unisici_soluzioni(T, S, i, n, k):
    ...
    # mi basta iterare sulle n soluzioni!
